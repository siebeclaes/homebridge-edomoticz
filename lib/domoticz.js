...
Domoticz.deviceStatus = function(accessory, completion, error) {
  this.deviceStatusByIdx(accessory.platform, accessory.idx, completion, error);
};

Domoticz.deviceStatusByIdx = function(platform, idx, completion, error) {
  if (baseHttpRequest === false) {
    return;
  }

  var url = platform.apiBaseURL + "type=devices&rid=" + idx;
  baseHttpRequest.get({
    url: url
  }, function(err, response, json) {
    if (!err && response.statusCode == 200 && json.result !== undefined)
